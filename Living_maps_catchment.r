#######################################################################################################
### Automated object-based classification using RF for catchment pioneer ##############################
#
#
...
#training data
training.data.habitat.shp <- readOGR("Training_Data/Catchment_FEP_OS_TrainingPoints_final_reduced.shp", "Catchment_FEP_OS_TrainingPoints_final_reduced")

segmentation.shp <- readOGR("Segmentation/Living_Maps_Segmentation_Cumbria.shp", "Living_Maps_Segmentation_Cumbria", useC=T)
...
S2_summer <- "S2/S2_Cumbria_Masked/Cumbria_S2_20160202_80_3_4_merge_mask.tif"
S2_winter <- "S2/S2_Cumbria_Masked/Cumbria_S2_20160314_80_3_4_merge_mask.tif"
height <- "Topography/Cumbria_EA_IHM/EA_IHM_2014_DTM_Resampled_10m_Subset.img"
slope <- "Topography/Cumbria_EA_IHM/EA_IHM_2014_DTM_Resampled_10m_Subset_SLOPE.img"
aspect <- "Topography/Cumbria_EA_IHM/EA_IHM_2014_DTM_Resampled_10m_Subset_ASPECT.img"
sar_summer <- "S1/S1_Cumbria/cumbria_2016_01_08.tif"
sar_winter <- "S1/S1_Cumbria/cumbria_2016_07_06.tif"
LSU_summer <- "LSU/Outputs/Cumbria_S2_20160602_80_3_4_merge_unmixed.tif"
LSU_winter <- "LSU/Outputs/Cumbria_S2_20160314_80_4_3_merge_unmixed.tif"
OS_VectorMap <- "OS/Cumbria_Rasters/Cumbria_VectorMap_District.tif"
OS_dist_building <- "OS/ProximityRasters/Prox_Cumbria_Building.tif"
OS_dist_foreshore <- "OS/ProximityRasters/Prox_Cumbria_Foreshore.tif"
OS_dist_road <- "OS/ProximityRasters/Prox_Cumbria_Roads.tif"
OS_dist_surfacewater <- "OS/ProximityRasters/Prox_Cumbria_SurfaceWater_Area.tif"
OS_dist_tidalwater <- "OS/ProximityRasters/Prox_Cumbria_TidalWater.tif"
OS_dist_woodland <- "OS/ProximityRasters/Prox_Cumbria_Woodland.tif"
bioclim_max_temp <- "Bioclim/Catchment/bio5_bioclim_max_temp.tif"
...
segmentation.raster <-raster("Segmentation/Living_Maps_Segmentation_Cumbria.tif")
...
#zonal_stats_seg <- write.table(zonal_stats_seg, "zonal_stats/Catchment/zonal_stats_seg_20170315.txt", sep="\t")
zonal_stats_seg <- read.table("zonal_stats/Catchment/zonal_stats_seg_20170315.txt", sep="\t", header=T)
...
write.table(zonal_stats_seg, "zonal_stats/Catchment/zonal_stats_seg_20170824.txt", sep="\t")
...
{
  zonal_stats_seg <- read.table("zonal_stats/Catchment/zonal_stats_seg_20170824.txt", sep="\t", header=T, as.is=T)
}
...
set.seed(9) # Set a random seed to ensure consistent training and test datasets
...
write.table(training.data, "training_data/Catchment/training_data_catchment.txt", sep="\t")
write.table(training.data.test, "training_data/Catchment/training_data_test_catchment.txt", sep="\t")
...
training.data <- read.table("training_data/Catchment/training_data_catchment.txt", sep="\t", header=T)
training.data.test <- read.table("training_data/Catchment/training_data_test_catchment.txt", sep="\t", header=T)
...
writeOGR(segmentation.p,
         "Outputs/FINAL/Living_Maps_Catchment_RF_Detailed_20170824.shp",
         "Living_Maps_Catchment_RF_Detailed_20170824",
